...
*/
function ambientFetch(input, init) {
...
*/
function classifyKimiOAuthFailure(error) {
	let current = error;
...
*/
function decodeJsonBody(buffer, headerEncoding) {
	const direct = tryJson(buffer.toString("utf8"));
	if (direct !== void 0) return direct;
	const declared = String(headerEncoding ?? "").trim().toLowerCase();
	const kinds = [];
	if (buffer.length >= 2 && buffer[0] === 31 && buffer[1] === 139) kinds.push("gzip");
...
const requestSignal = (signal) => signal === void 0 ? AbortSignal.timeout(REQUEST_TIMEOUT_MS) : AbortSignal.any([signal, AbortSignal.timeout(REQUEST_TIMEOUT_MS)]);
/**
...
async function requestJson(fetchImpl, url, init, signal) {
	...
	return {
		response,
		payload: decodeJsonBody(buffer, response.headers.get("content-encoding"))
	};
}
...
function readDeviceAuthorization(payload) {
	...
	if (typeof deviceCode !== "string" || deviceCode === "" || typeof userCode !== "string" || userCode === "" || typeof verificationUri !== "string" || verificationUri === "" || typeof verificationUriComplete !== "string" || verificationUriComplete === "") throw new KimiOAuthError("oauth/response", "Kimi Code device authorization response is missing fields");
...
function readToken(payload, operation, previousRefresh) {
	...
	const expiresIn = Number(payload?.expires_in);
	if (typeof access !== "string" || access === "" || typeof refresh !== "string" || refresh === "" || !Number.isFinite(expiresIn) || expiresIn <= 0) throw new KimiOAuthError("oauth/response", `Kimi Code token ${operation} response is missing fields`);
	return {
		type: "oauth",
		access,
		refresh,
...
*/
function createKimiOAuth({ fetchImpl = (...args) => globalThis.fetch(...args), oauthHost = KIMI_OAUTH_HOST, now = Date.now, pause = sleep } = {}) {
	...
	const exchange = async (device, signal) => {
		...
		if (response.ok) {
			if (typeof payload?.access_token === "string") return {
				kind: "token",
				credential: readToken(payload, "poll", void 0)
			};
		...
		if (error === "expired_token") throw new KimiOAuthError("oauth/expired", "Kimi Code device authorization expired. Please restart login.");
		if (error === "access_denied") throw new KimiOAuthError("oauth/denied", "Kimi Code login was denied");
		if (payload === void 0) throw new KimiOAuthError("oauth/response", `Kimi Code device token response could not be read (status ${response.status})`);
	...
	const pollForToken = async (device, signal) => {
		const deadline = now() + device.expiresInSeconds * 1e3;
		let intervalMs = Math.max(1, device.intervalSeconds) * 1e3;
		let failures = 0;
		...
		for (;;) {
			...
			try {
				...
				if (!(error instanceof KimiOAuthError && error.retryable === true) || failures >= EXCHANGE_MAX_RETRIES) throw error;
				failures += 1;
				await pause(1e3 * 2 ** (failures - 1), signal);
				continue;
			}
			if (outcome.kind === "token") return outcome.credential;
			if (outcome.kind === "slow_down") intervalMs += 5e3;
			await pause(intervalMs, signal);
		}
	};
	return Object.freeze({
		...
		async login(interaction) {
			...
			if (!response.ok) throw new KimiOAuthError("oauth/unreachable", `Kimi Code device authorization failed (status ${response.status})${describeOauthError(payload)}`);
			if (payload === void 0) throw new KimiOAuthError("oauth/response", "Kimi Code device authorization response could not be read");
		...
		async refresh(credential, signal) {
			...
			for (let attempt = 0; attempt <= EXCHANGE_MAX_RETRIES; attempt += 1) {
				if (attempt > 0) await pause(1e3 * 2 ** (attempt - 1), signal);
				if (signal?.aborted) throw abortError(signal.reason);
				const { response, payload } = await requestJson(fetchImpl, `${host}/api/oauth/token`, {
					method: "POST",
					headers: { "Content-Type": "application/x-www-form-urlencoded" },
				...
				if (response.ok && typeof payload?.access_token === "string") return readToken(payload, "refresh", credential.refresh);
				if (response.status === 401 || response.status === 403 || payload?.error === "invalid_grant") throw new KimiOAuthError("oauth/refresh-rejected", `Kimi Code token refresh unauthorized (status ${response.status})${describeOauthError(payload)}`);
				if ((response.status === 429 || response.status >= 500) && attempt < EXCHANGE_MAX_RETRIES) {
...
*/
function explainKimiModelEntitlement(message) {
	const text = typeof message === "string" ? message : "";
	if (text === "") return void 0;
	const match = MODEL_ACCESS_REJECTION.exec(text);
	const model = match === null ? void 0 : match[1].replace(/[.\-_]+$/u, "");
	if (model === void 0 && !TIER_REJECTION.test(text)) return void 0;
	const subject = model === void 0 ? "当前 Kimi Code 订阅档位不包含本次请求的模型" : `当前 Kimi Code 订阅档位不包含模型「${model}」`;
	const named = model === void 0 ? "this model" : `"${model}"`;
	return `${subject}，请改用 ${FALLBACK_MODEL}（K2.8 Preview），或升级 Kimi Code 档位。这不是密钥或登录问题。 / Your Kimi Code plan does not include ${named}; select ${FALLBACK_MODEL} (K2.8 Preview) or upgrade your plan. This is a plan limit, not a credential problem.`;
}
const TERMINAL_EVENTS = /* @__PURE__ */ new Set(["error", "done"]);
/**
...
const withK28PreviewMetadata = (model) => model.id === "kimi-for-coding" ? Object.freeze({
	...model,
	...K28_PREVIEW_MODEL_PATCH
}) : model;
/**
...
*/
function createKimiSubscriptionProvider({ onAuthRejected, oauth, fetchImpl, oauthHost } = {}) {
...
const SEARCH_PROVIDER_CHOICES$1 = Object.freeze([
	SEARCH_PROVIDER_DEFAULT,
	SEARCH_PROVIDER_AUTO,
	SEARCH_PROVIDER_KIMI
]);
const choice = () => z.union(SEARCH_PROVIDER_CHOICES$1).default(SEARCH_PROVIDER_DEFAULT);
/**
...
*/
const Config = z.object({ [SEARCH_PROVIDER_FIELD]: typeof choice().volatile === "function" ? choice().volatile() : choice() });
const readChoice = (value) => SEARCH_PROVIDER_CHOICES$1.includes(value) ? value : SEARCH_PROVIDER_DEFAULT;
const currentChoice = (config) => {
	const field = config?.[SEARCH_PROVIDER_FIELD];
	const value = field !== null && typeof field === "object" && typeof field.get === "function" ? field.get() : field;
...
*/
function createSearchSettings(ctx, config) {
	if (typeof ctx.settings?.register === "function") return ctx.settings.register(SETTINGS_NAMESPACE, z.object({ [SEARCH_PROVIDER_FIELD]: choice() }));
	...
	if (typeof ctx.settings?.configure === "function") ctx.effect?.(() => ctx.settings.configure({ auto: false }, ctx.fiber));
	...
	return {
		...
		update: async (patch) => {
			...
			if (value !== patch?.["searchProvider"]) throw new Error("Invalid search provider preference");
			const entryId = ctx.fiber?.entry?.options?.id ?? "kimi-subscription";
